...
//ПолныйАдресРесурса = "https://zabbix.org:443/zabbix/api_jsonrpc.php";
//СтруктураURI = СтруктураURI(ПолныйАдресРесурса);

HTTPСоединение = Новый HTTPСоединение("zabbix.org", 443,,,ИнтернетПрокси,120,ssl);
	
ЗаголовокЗапросаHTTP = Новый Соответствие();
ЗаголовокЗапросаHTTP.Вставить("Content-Type", "application/json;charset=UTF-8");	
HTTPЗапрос = Новый HTTPЗапрос("zabbix/api_jsonrpc.php", ЗаголовокЗапросаHTTP); 
...
ЗаписьJSON.ЗаписатьНачалоОбъекта();
	...
	ЗаписьJSON.ЗаписатьЗначение("apiinfo.version");
	
	ЗаписьJSON.ЗаписатьИмяСвойства("id");
	ЗаписьJSON.ЗаписатьЗначение(0);
	
	ЗаписьJSON.ЗаписатьИмяСвойства("auth");
